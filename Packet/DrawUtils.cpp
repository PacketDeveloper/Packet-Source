...
void initializeSigs() {
	tess_vertex = reinterpret_cast<tess_vertex_t>(FindSignature("40 57 48 83 EC ?? 0F 29 74 24 ?? 0F"));
...
void DrawUtils::drawText(vec2_t pos, std::string* textStr, MC_Color color, float textSize, float alpha, bool hasShadow) {
	...
	if (hasShadow) {
		...
		int colorPos = -1;
		while (*textToDraw != '\0') {
			if (*textToDraw == (char)194 && textToDraw[1] != 0 && textToDraw[1] == (char)167 && textToDraw[2] != 0 && textToDraw[2] == '7') {
...
void DrawUtils::drawImage(std::string FilePath, vec2_t& imagePos, vec2_t& ImageDimension, vec2_t& idk) {
	...
	if (flushImageAddr == 0x0)
		logF("drawImage sig not working :(");
