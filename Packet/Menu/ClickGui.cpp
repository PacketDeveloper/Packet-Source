...
static constexpr float textSize = 0.95;
...
void ClickGui::renderCategory(Category category, bool Rainbow, float backgroundAlpha) {
		...
		if (clickGUI->animations) {
			if (ourWindow->isExtended) {
				if (clickGUI->openAnimation == true) {
					ourWindow->animation = 70.f;
					ourWindow->isInAnimation = true;
					if (clickGUI->headerAnimation == true) {  // Fixes animation when right clicking category header
	...
	std::vector<std::shared_ptr<IModule>> moduleList;
	//Sorting by lenght, that is terrible but it works
	{
