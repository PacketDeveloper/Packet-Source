...
float bgOpacity = 0.10f;

Radar::Radar() : IModule(0, Category::VISUAL, "Radar") {
	registerBoolSetting("Grid", &grid, true);
	registerFloatSetting("Opacity", &bgOpacity, bgOpacity, 0.1f, 1.0f);
	registerFloatSetting("Zoom", &zoom, zoom, 0.6f / 2.0f, 2.0f);
	registerIntSetting("Size", &size, size, 50, 100);
