...
void C_Inventory::dropSlot(int slot) {
	...
	static drop_t func = reinterpret_cast<drop_t>(FindSignature("85 D2 0F 88 ?? ?? ?? ?? 48 89 5C 24 ?? 55")); // credit to floppy dolphin/nrg for signature
	if (func != 0)
		func(this, slot, 0);
}
...
void C_ContainerScreenController::handleAutoPlace(uintptr_t a1, std::string name, int slot) {
	...
	static ContainerScreenController__autoPlace autoPlaceFunc = reinterpret_cast<ContainerScreenController__autoPlace>(FindSignature("40 55 53 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 ?? 48 81 EC ?? ?? ?? ?? 48 C7 44 24 ?? ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 07 4D 8B F9 4D 8B E8")); // credit to floppy dolphin/nrg for signature
