...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			...
			//g_Data.getGuiData()->displayClientMessageF("1", GRAY, WHITE, GREEN, !configExists ? "" : "", GRAY, name.c_str(), GREEN);
			if (!helpedUser && name != "default") {
				helpedUser = true;
				//g_Data.getGuiData()->displayClientMessageF("2", GRAY, WHITE, GRAY, WHITE, cmdMgr->prefix, YELLOW);
			}
		}
	} else {
		if (g_Data.getLocalPlayer() != nullptr) 
			g_Data.getGuiData()->displayClientMessageF("3", GRAY, WHITE, RED, GRAY, name.c_str(), RED);
	}
