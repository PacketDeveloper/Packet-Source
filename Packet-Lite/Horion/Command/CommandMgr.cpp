...
void CommandMgr::initCommands() {
	...
	//mmandList.push_back(new RelativeTeleportCommand());
	...
	//mmandList.push_back(new PathCommand());
		...
		for (auto it = this->commandList.begin(); it != this->commandList.end(); ++it) {
			...
			for (auto it = aliases->begin(); it != aliases->end(); ++it) {
				if (*it == cmd) {
					try {
						if (!c->execute(&args))
							g_Data.getClientInstance()->getGuiData()->displayClientMessageF( c->getCommand(), c->getUsage(cmd.c_str()));
					} catch (...) {
						g_Data.getClientInstance()->getGuiData()->displayClientMessageF( c->getCommand(), c->getUsage(cmd.c_str()));
					}
					return;
