...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	static constexpr float GrayscaleIncrease = 0.10f;
	static float GrayscaleColor = 0.f;
	static bool isIncreasingGrayscale = true;
	static bool isIncreasingcurrGrayscale = true;
	static float currGrayscaleColor = 0.f;
	currGrayscaleColor = GrayscaleColor;
	...
	{
		GrayscaleColor += (isIncreasingGrayscale ? 0.005 : -0.005);
		if (GrayscaleColor < 0.3f) {
			GrayscaleColor = 0.3f;
			isIncreasingGrayscale = true;
		}
		if (GrayscaleColor > 1.f) {
			GrayscaleColor = 1.f;
			isIncreasingGrayscale = false;
		}
	}
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
...

			{
				...
				if (shouldRenderWatermark) {
					...
					if (hudModuleTwo->dynamic) {
						DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name, MC_Color(currGrayscaleColor, currGrayscaleColor, currGrayscaleColor), nameTextSize);
						DrawUtils::drawText(vec2_t(rect.x + borderPadding + nameLength, rect.w - 7), &version, MC_Color(currGrayscaleColor, currGrayscaleColor, currGrayscaleColor), versionTextSize);
					}
					if (hudModuleTwo->white) {
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						vec4_t leftRect = vec4_t(
							...
							xOffset - 1,
							yOffset + textPadding * 2 + textHeight);
						vec4_t bar = vec4_t(  // Bar
							xOffset - 10,
							yOffset,
							xOffset - 9,
							yOffset + textPadding * 2 + textHeight * 2);

						underline = vec4_t(windowSize.x - (lastTextLenght + 2.f + (textPadding * 2.f)), leftRect.y, leftRect.x, leftRect.y + 1.f);
						//bar = vec4_t(windowSize.x - (lastTextLenght + 3.f), leftRect.y, leftRect.x, leftRect.y + 4.f);
						...
						if (hudModuleTwo->r2) {
							Utils::ColorConvertRGBtoHSV(rcolors[0 & 1], rcolors[2], rcolors[01], currColor[0], currColor[1], currColor[2]);
							currColor[0] += 1.f / a * c;
							Utils::ColorConvertHSVtoRGB(currColor[0 & 1], currColor[2], currColor[2], currColor[0], currColor[1], currColor[1]);
						}
						if (hudModuleTwo->horion) {
							Utils::ColorConvertRGBtoHSV(rcolors[0], rcolors[1], rcolors[2], currColor[0], currColor[1], currColor[2]);
							currColor[0] += 1.f / a * c;
							Utils::ColorConvertHSVtoRGB(currColor[0], currColor[1], currColor[2], currColor[0], currColor[1], currColor[2]);
						}
						if (hudModuleTwo->dynamic) {
							currGrayscaleColor += (isIncreasingcurrGrayscale ? GrayscaleIncrease : -GrayscaleIncrease);
							if (currGrayscaleColor < 0.3f) {
								currGrayscaleColor = 0.3f;
								isIncreasingcurrGrayscale = true;
							}
							if (currGrayscaleColor > 1.f) {
								currGrayscaleColor = 1.f;
								isIncreasingcurrGrayscale = false;
							}
						}
						...
						// ArrayList Modes
						if (hudModule->arraylistOutline) {  // Outline
							if (!hudModuleTwo->dynamic) {
							...
							} else if (hudModuleTwo->dynamic) {
								DrawUtils::fillRectangle(leftRect, MC_Color(currGrayscaleColor, currGrayscaleColor, currGrayscaleColor), 1.f);   //Line around modules
								DrawUtils::fillRectangle(underline, MC_Color(currGrayscaleColor, currGrayscaleColor, currGrayscaleColor), 1.f);  //underline
							}
							if (hudModuleTwo->white) {
						...
						if (hudModule->arBar) {
							// https://cdn.discordapp.com/attachments/809338249918939167/857265727483281418/unknown.png <- example
						}

						underline = vec4_t(windowSize.x - (textWidth + 2.f + (textPadding * 2.f)), leftRect.w, windowSize.x, leftRect.w + 1.f);
						bar = vec4_t(windowSize.x - (textWidth + 2.f + (textPadding * 2.f)), leftRect.w, windowSize.x, leftRect.w + 1.f);
						...
						if (hudModuleTwo->dynamic) {
							DrawUtils::drawText(textPos, &textStr, MC_Color(currGrayscaleColor, currGrayscaleColor, currGrayscaleColor), textSize);
						} else if (hudModuleTwo->white) {
...
void Hooks::Actor_lerpMotion(C_Entity* _this, vec3_t motVec) {
	...
	if (noKnockbackmod->isEnabled()) {
		...
		static auto flightMod = moduleMgr->getModule<Glide>();
		...
		static auto invmoveMod = moduleMgr->getModule<InventoryMove>();
		static auto hiveflyMod = moduleMgr->getModule<HiveFly>();
		if (noKnockbackmod->isEnabled()) {
					...
					if (!freecamMod->isEnabled()) {
						if (!scaffoldMod->isEnabled()) {
							if (!longjumpMod->isEnabled()) {
								if (!invmoveMod->isEnabled()) {
									if (!hiveflyMod->isEnabled()) {
...
float Hooks::GetGamma(uintptr_t* a1) {
	...
	if (cameraMod->fullbrightMode && cameraMod->isEnabled())
		return 25.f;
...
void Hooks::LocalPlayer__updateFromCamera(__int64 a1, C_Camera* camera) {
	...
	auto noHurtcamMod = moduleMgr->getModule<CameraMod>();
