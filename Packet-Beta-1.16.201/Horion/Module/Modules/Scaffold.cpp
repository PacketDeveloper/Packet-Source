...
bool Scaffold::tryScaffold(vec3_t blockBelow) {
	...
	if (blockLegacy->material->isReplaceable) {
		...
		bool foundCandidate = true;  // Place on air
		int i = 0;
		for (auto current : checklist) {
			vec3_ti calc = blok.sub(*current);
...
void Scaffold::onTick(C_GameMode* gm) {
	if (strcmp(g_Data.getRakNetInstance()->serverIp.getText(), "geo.hivebedrock.network") == 0) {  // The Hive
	...
	if (this->expandModeBool) {
		if (this->staircaseMode && !GameData::isKeyDown(*input->sneakKey)) {
			...
			blockBelow.y -= g_Data.getLocalPlayer()->height;
			blockBelow.x = blockBelow.x += cos(cal) * expand;  // Extend ahead the player X
			blockBelow.z = blockBelow.z += sin(cal) * expand;  // Extend ahead the player Z
